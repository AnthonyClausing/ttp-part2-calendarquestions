...
export const handleNav = function (event) {
  if (event.target.className === 'next') {
    if (this.props.month == 12) {
      this.props.handleEvents(this.props.year + 1, 1)
      ...
      this.props.handleEvents(this.props.year, this.props.month + 1)
    ...
    if (this.props.month == 1) {
      if(this.props.year == 101) return alert('cannot keep track of events before year 101')
        this.props.handleEvents(this.props.year - 1, 12)
...
//For comparing start Times to sort events on same day
export const getTimeStart = function (event) {
  let startTime = event.time.slice(0, 8)
  let changedNumber = ''
